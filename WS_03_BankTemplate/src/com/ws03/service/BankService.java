...
public class BankService {
	...
	/**
	 * userSeq의 해당하는 accountList 인덱스 갯수 리턴
	 * @param userSeq
	 * @return accountList의 인덱스 갯수 리턴
	 */
	public int selectByUserSeqFromIndexCount(int userSeq) {
		int accListIndex=0;
	...
	/**
	 * userSeq를 이용해서 accountList에서 해당하는 인덱스번호 리턴
	 * @param userSeq
	 * @return accountList의 인덱스 번호 리턴 | 배열의 값이 -1이면 없음.
	 * 
	 */
	public int[] selectByUserSeqFromIndex(int userSeq) {
		int[]index =new int[ACCOUNT_CURRENT_SIZE];
		for(int i =0;i<ACCOUNT_CURRENT_SIZE;i++) {//-1로 초기화하기위함
			index[i]=-1;
		}
		int indexSize=0;
		for (int i=0;i<ACCOUNT_CURRENT_SIZE;i++) {
	...
	*/
	public AccountDto[] getAccountList(int userSeq) {
		int index = selectByUserSeqFromIndexCount(userSeq);
		if (index == 0) return null;
		AccountDto[] aDtos= new AccountDto[index];
		int sb[] = selectByUserSeqFromIndex(userSeq);
		//배열의 값이 0이상이면 값이있다.
		int a=0;
		for(int j=0;j<ACCOUNT_CURRENT_SIZE;j++) {
			if (sb[j]>-1) {
				aDtos[a++]=accountList[sb[j]];	
			}
